...
async def login_menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if ud and 'session_string' in ud:
        keyboard = [[InlineKeyboardButton("🔒 Logout", callback_data="logout")], [InlineKeyboardButton("« Back", callback_data="main_menu")]]
        await query.edit_message_text("You are already logged in.", reply_markup=InlineKeyboardMarkup(keyboard))
        return SELECTING_ACTION
    else:
        keyboard = [[InlineKeyboardButton("« Back", callback_data="main_menu")]]
        await query.edit_message_text("You are not logged in. Please send your Telethon session string to log in.\n\nUse /stop to cancel.", reply_markup=InlineKeyboardMarkup(keyboard))
...
async def handle_login_session(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        async with get_userbot_client(session_string) as client:
            ...
            await msg.edit_text(f"✅ Successfully logged in as <b>{me.first_name}</b>!", parse_mode=ParseMode.HTML, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("« Back to Settings", callback_data="main_menu")]]))
    except Exception as e:
        await msg.edit_text(f"❌ Login failed. Error: {e}", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("« Back to Settings", callback_data="main_menu")]]))
    return SELECTING_ACTION
...
async def targets_menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    keyboard = [[InlineKeyboardButton(f"🗑️ {t['name']} ({t['id']})", callback_data=f"delete_target_{t['id']}")] for t in targets]
...
async def add_target_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    query = update.callback_query
    await query.answer()
    await query.edit_message_text("Please send me a short, memorable name for this target (e.g., 'My Main Group').\n\nUse /stop to cancel.")
...
async def handle_target_id(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        if target_id.lower() != 'me':
            ...
            if not any(admin.user.id == context.bot.id for admin in await context.bot.get_chat_administrators(chat_id=target_id)):
                 raise Exception("I am not an admin in that chat.")
        await db.add_target(update.effective_user.id, target_name, target_id)
        await msg.edit_text(f"✅ Target '{target_name}' added successfully!", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("« Back to Settings", callback_data="main_menu")]]))
    except Exception as e:
        await msg.edit_text(f"❌ Failed to verify target.\n<b>Error:</b> {e}", parse_mode=ParseMode.HTML, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("« Back to Settings", callback_data="main_menu")]]))
    return SELECTING_ACTION
...
async def delete_target_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    keyboard = [[InlineKeyboardButton("✅ Yes, Delete It", callback_data=f"confirm_delete_target_{target_id}")], [InlineKeyboardButton("❌ No, Keep It", callback_data="targets_menu")]]
    await query.edit_message_text(f"Are you sure you want to delete the target with ID `{target_id}`?", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode=ParseMode.MARKDOWN)
...
async def add_worker_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    keyboard = [[InlineKeyboardButton(t['name'], callback_data=f"select_worker_target_{t['id']}")] for t in targets]
    keyboard.append([InlineKeyboardButton("« Back", callback_data="workers_menu")])
    await query.edit_message_text("Please choose a target group to add the worker(s) to:", reply_markup=InlineKeyboardMarkup(keyboard))
...
async def handle_worker_tokens(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    target_group_id = int(context.user_data.pop('worker_target_id'))
...
async def scrape_command_entry(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not targets:
        await update.message.reply_text("You have no targets. Please add one via the settings menu.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⚙️ Open Settings", callback_data="main_menu")]]))
        return ConversationHandler.END
...
async def scrape_upload_as_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if context.user_data['scrape_type'] == 'deep':
        await query.edit_message_text("Please specify the range of links to process (e.g., `1-77` or `all`).")
...
async def start_deep_scrape(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = await context.bot.send_message(update.effective_chat.id, f"Scanning `{url}` for links...", parse_mode=ParseMode.MARKDOWN)
    try:
        response = await asyncio.to_thread(requests.get, url, headers={'User-Agent': 'Mozilla/5.0'})
        soup = BeautifulSoup(response.content, 'html.parser')
        links = sorted(list({urljoin(url, a['href']) for a in soup.find_all('a', href=True) if urljoin(url, a.get('href', '')) != url and not (urljoin(url, a.get('href', ''))).endswith(('.zip', '.rar', '.exe', '.pdf'))}))
